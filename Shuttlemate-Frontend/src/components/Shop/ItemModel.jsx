    ...
    try {
      ...
      if (itemDetails.itemphoto) {
        const storageRef = ref(
          storage,
          `items/${selectedShop._id}/${selectedCategory._id}/${Date.now()}_${itemDetails.itemphoto.name}`,
        );
  ...
  const submitItemToAPI = async (itemData) => {
    try {
      ...
      const response = await axios.post(
        `${window.__API_BASE_URL__}/api/shops/shop/${selectedShop._id}/categories/${selectedCategory._id}/items`,
        itemData,
        {
          headers: {
      ...
      <div className="p-6 overflow-y-auto bg-white rounded-lg max-h-[90vh] w-auto">
        ...
        {selectedShop && selectedCategory && (
          <div className="mb-4 text-sm text-gray-600">
            ...
            category in{" "}
            <span className="font-semibold">{selectedShop.ShopName}</span> shop.
          </div>
        )}
