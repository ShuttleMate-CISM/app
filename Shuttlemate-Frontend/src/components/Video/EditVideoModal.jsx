  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (changingCreatorPhoto && formData.videoCreatorPhoto) {
        creatorPhotoURL = await uploadFile(
      ...
      const response = await axios.put(
        `${window.__API_BASE_URL__}/api/videos/video/${videoData._id}`,
        {
          videoUrl: videoURL,
          imgUrl: thumbnailURL,
      ...
      setUploadError(
        "An error occurred while updating the video. Please try again.",
      );
    } finally {
      setUploading(false);
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            ...
            <label className="block mb-2 text-sm text-gray-600">
              Upload new creator photo (optional)
            </label>
            <input
              type="file"
