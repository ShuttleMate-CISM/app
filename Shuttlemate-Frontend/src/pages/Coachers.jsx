...
const Coachers = () => {
  ...
  const fetchCoachers = async () => {
    try {
      const response = await axios.get(
        `${window.__API_BASE_URL__}/api/coachers`,
      );
  ...
  const fetchCourts = async () => {
    try {
      const response = await axios.get(`${window.__API_BASE_URL__}/api/courts`);
  ...
  const fetchCoacherById = async (id) => {
    try {
      const response = await axios.get(
        `${window.__API_BASE_URL__}/api/Coachers/${id}`,
      );
    ...
    if (step === 1) {
      ...
      if (needsPhotoUpload) {
        ...
        const storageRef = ref(
          storage,
          `coaches/${Date.now()}_${formData.CoachPhoto.name}`,
        );
  ...
  const submitCoachData = (coachData) => {
    ...
    if (editingCoachId) {
      ...
      axios
        .put(
          `${window.__API_BASE_URL__}/api/Coachers/${editingCoachId}`,
          coachData,
        )
        .then(() => {
          fetchCoachers();
      ...
      // Add new coach
      axios
        .post(`${window.__API_BASE_URL__}/api/coachers`, coachData)
        .then(() => {
          fetchCoachers();
          toggleModal();
          setUploading(false);
  ...
  const handleDeleteCoacher = async (id) => {
    try {
      ...
      if (result.isConfirmed) {
        await axios.delete(
          `${window.__API_BASE_URL__}/api/Coachers/coach/${id}`,
        );
  ...
  const handleUpdateCoacher = (updatedCoacher) => {
    axios
      .put(
        `${window.__API_BASE_URL__}/api/Coachers/${updatedCoacher._id}`,
        updatedCoacher,
      )
      .then((response) => {
        fetchCoachers();
