...
const Home = () => {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        const endpoints = {
          users: `${window.__API_BASE_URL__}/api/user`,
          coaches: `${window.__API_BASE_URL__}/api/Coachers`,
          courts: `${window.__API_BASE_URL__}/api/courts`,
          videos: `${window.__API_BASE_URL__}/api/videos`,
          shops: `${window.__API_BASE_URL__}/api/shops`,
          matches: `${window.__API_BASE_URL__}/api/matches`,
          items: `${window.__API_BASE_URL__}/api/items`,
        };
        ...
        const promises = Object.entries(endpoints).map(
          async ([key, endpoint]) => {
            try {
              ...
              if (!response.ok) {
                throw new Error(
                  `Failed to fetch ${key}: ${response.statusText}`,
                );
              }
  ...
  const adminCards = [
    ...
    },
    {
      ...
      value: apiData.loading
        ? "Loading..."
        : apiData.totalVideos.toLocaleString(),
      change: "+8.3%",
      icon: Video,
      trend: "up",
    ...
    },
    {
      ...
      value: apiData.loading
        ? "Loading..."
        : apiData.totalCoaches.toLocaleString(),
      change: "+15.4%",
      icon: UserCheck,
      trend: "up",
      bgColor: "bg-green-50",
    ...
    },
    {
      ...
      value: apiData.loading
        ? "Loading..."
        : apiData.totalCourts.toLocaleString(),
      change: "+6.7%",
      icon: MapPin,
      trend: "up",
    ...
    },
    {
      ...
      value: apiData.loading
        ? "Loading..."
        : apiData.totalMatches.toLocaleString(),
      change: "+11.2%",
      icon: Activity,
      trend: "up",
          ...
          <div className="p-6 bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900 mb-1">
                {apiData.loading
                  ? "Loading..."
                  : apiData.totalShops.toLocaleString()}
              </p>
          ...
          <div className="p-6 bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900 mb-1">
                {apiData.loading
                  ? "Loading..."
                  : apiData.totalItems.toLocaleString()}
              </p>
