...
const Users = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch(`${window.__API_BASE_URL__}/api/auth/users`);
  ...
  const handleDeleteUser = async (userId) => {
    ...
    try {
      const response = await fetch(
        `${window.__API_BASE_URL__}/api/auth/delete/${userId}`,
        {
          method: "DELETE",
          headers: {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Navbar />
      <h1 className="pt-10 text-4xl font-bold text-center pb-7">
        Registered users
      </h1>
      {/* User Table */}
      <UserTable
