...
const Courts = () => {
  ...
  const fetchCoacherById = async (id) => {
    try {
      const response = await axios.get(
        `${window.__API_BASE_URL__}/api/courts/${id}`,
      );
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
            ...
            if (editingCourtId) {
              axios
                .put(
                  `${window.__API_BASE_URL__}/api/courts/court/${editingCourtId}`,
                  updatedCourt,
                )
                .then(() => {
                  fetchCourts();
              ...
              axios
                .post(`${window.__API_BASE_URL__}/api/courts`, updatedCourt)
                .then(() => {
                  fetchCourts();
                  toggleModal();
                  setUploading(false);
  ...
  const handleDeleteCourt = async (id) => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        await axios.delete(`${window.__API_BASE_URL__}/api/courts/${id}`);
  ...
  const handleUpdateCourt = (updatedCourt) => {
    axios
      .put(
        `${window.__API_BASE_URL__}/api/courts/court/${updatedCourt._id}`,
        updatedCourt,
      )
      .then((response) => {
        fetchCourts();
