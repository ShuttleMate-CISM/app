...
const Payments = () => {
            ...
            if (payment.userId) {
              try {
                const userResponse = await fetch(
                  `${window.__API_BASE_URL__}/api/users/${payment.userId}`,
                );
                if (userResponse.ok) {
            ...
            if (payment.itemId) {
              try {
                const itemResponse = await fetch(
                  `${window.__API_BASE_URL__}/api/shops/item/${payment.itemId}`,
                );
                if (itemResponse.ok) {
  ...
  const fetchPayments = async (status = "") => {
    try {
      ...
      console.log(
        ...
        `${window.__API_BASE_URL__}/api/payment/payments?${queryParams}`,
      );
    ...
    try {
      ...
      try {
        const statsResponse = await fetch(
          `${window.__API_BASE_URL__}/api/payment/payments/stats`,
        );
        if (statsResponse.ok) {
        ...
        console.log(
          "Stats endpoint not available, calculating from payments list",
        );
      }

      // Fallback: calculate from payments list
      const response = await fetch(
        `${window.__API_BASE_URL__}/api/payment/payments?limit=1000`,
      );
      ...
      if (Array.isArray(allPayments)) {
        ...
        console.error(
          "Stats: Expected array of payments, got:",
          typeof allPayments,
        );
      }
    } catch (err) {
  ...
  const handleDeletePayment = async (paymentId) => {
    ...
    try {
      const response = await fetch(
        `${window.__API_BASE_URL__}/api/payment/payment/${paymentId}`,
        {
          method: "DELETE",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to delete payment: ${response.status} ${response.statusText}`,
        );
      }
      ...
      setEnrichedPayments((prevPayments) =>
        prevPayments.filter((payment) => payment._id !== paymentId),
      );
  ...
  const handleUpdatePaymentStatus = async (paymentId, newStatus) => {
    try {
      ...
      const response = await fetch(
        `${window.__API_BASE_URL__}/api/payment/payment/${paymentId}/status`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
