...
All 21 findings share the same rule: **`jssecurity:S5147`** — *"Change this code to not construct database queries directly from user-controlled data"* (NoSQL Injection).
...
const users = await User.find({})
  .select('-password -failedLoginAttempts -accountLockedUntil -__v');
